...
dir.create(paste0("./lassosum2/prs/tuning/rho_",rho,"_size_",size), recursive=T)
dir.create(paste0("./lassosum2/prs/validation/rho_",rho,"_size_",size), recursive=T)

nsnps <- integer()
for (chr in 1:22){
  #prs.file <- fread2(paste0("./lassosum2/coef/rho_",rho,"_size_",size,"_lassosum2-chr",chr,".txt"))
  nsnps[chr] <- 122
}
...
for (race1 in c('AFR','AMR','EAS','EUR','SAS')){
  ...
  for (chr in 1:22){
    ...
    system(paste0("/dcs04/nilanjan/data/jzhang2/TOOLS/plink/plink2 --threads ",NCORES,
                  ...
                  " --score ../../",race1,"/rep_",rep,"/lassosum2/coef/rho_",rho,"_size_",ifelse(race1=='EUR',4,size),"_lassosum2-chr",chr,".txt",
                  " cols=+scoresums,-scoreavgs --score-col-nums 3-", nsnps[chr],
                  " --out ./lassosum2/prs/tuning/rho_",rho,"_size_",size,"/",race,"-",race1,"_chr", chr,"_tuning_score"))

    score <- fread2(paste0("./lassosum2/prs/tuning/rho_",rho,"_size_",size,"/",race,"-",race1,"_chr", chr,"_tuning_score.sscore"))
  ...
  SCORE <- SCORE_all
  save(SCORE, SCORE_id, file = paste0("./lassosum2/prs/tuning/rho_",rho,"_size_",size,"/",race,"-",race1,"_tuning_score.RData"))
...
for (race1 in c('AFR','AMR','EAS','EUR','SAS')){
  ...
  for (chr in 1:22){
    ...
    system(paste0("/dcs04/nilanjan/data/jzhang2/TOOLS/plink/plink2 --threads ",NCORES,
                  ...
                  " --score ../../",race1,"/rep_",rep,"/lassosum2/coef/rho_",rho,"_size_",ifelse(race1=='EUR',4,size),"_lassosum2-chr",chr,".txt",
                  " cols=+scoresums,-scoreavgs --score-col-nums 3-",nsnps[chr],
                  " --out ./lassosum2/prs/validation/rho_",rho,"_size_",size,"/",race,"-",race1,"_chr", chr,"_validation_score"))

    score <- fread2(paste0("./lassosum2/prs/validation/rho_",rho,"_size_",size,"/",race,"-",race1,"_chr", chr,"_validation_score.sscore"))
    if(chr==1){
      SCORE_all <- score[, -1:-4,drop=F]
      SCORE_id <- score[,1:2,drop=F]
    }else{
  ...
  SCORE <- SCORE_all
  save(SCORE, SCORE_id, file = paste0("./lassosum2/prs/validation/rho_",rho,"_size_",size,"/",race,"-",race1,"_validation_score.RData"))
  rm(list=c("SCORE","SCORE_id","score","SCORE_all"))
}
