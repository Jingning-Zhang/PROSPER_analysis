...
for(i in 1:length(race1_all) ){
  race1 <- race1_all[i]
  best <- c(best, readRDS(paste0("../../",race1,"/rep_",rep,"/lassosum2/prs/results/rho_",rho,"_size_",ifelse(race1=='EUR',4,size),"/",race1,"_best_param.rds")))
}
...
for(i in 1:length(race1_all) ){
  race1 <- race1_all[i]
  load(paste0("./lassosum2/prs/tuning/rho_",rho,"_size_",size,"/",race,"-",race1,"_tuning_score.RData"))
  if(race1=='AFR'){
    id <- SCORE_id
    SCORE_all <- SCORE
...
for(i in 1:length(race1_all) ){
  race1 <- race1_all[i]
  load(paste0("./lassosum2/prs/validation/rho_",rho,"_size_",size,"/",race,"-",race1,"_validation_score.RData"))
  if(race1=='AFR'){
    id <- SCORE_id
    SCORE_all <- SCORE
    SCORE_best <- SCORE[,best[race1]]
...
saveRDS(res, paste0("./lassosum2/prs/results/rho_",rho,"_size_",size,"/",race,"_validation_R2_sl.rds"))

res_model <- list(sl=sl, wt = wt)
saveRDS(res_model, paste0("./lassosum2/prs/results/rho_",rho,"_size_",size,"/",race,"_sl_models.rds"))
