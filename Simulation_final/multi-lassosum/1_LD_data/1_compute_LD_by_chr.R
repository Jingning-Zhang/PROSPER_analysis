...
option_list = list(
  ...
  make_option("--refgeno", action="store", default=NA, type='character',
              help=" [required]"),
  make_option("--snplist", action="store", default=NULL, type='character',
              help=" [required]"),
...
if(!is.null(opt$snplist)){
  commgeno <- paste0(" --bfile ",opt$refgeno, chr, " --extract ",opt$snplist, " --force-intersect")
}else{
  commgeno <- paste0(" --bfile ",opt$refgeno, chr)
}
