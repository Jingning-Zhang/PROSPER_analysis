...
dir.create("/dcs04/nilanjan/data/jzhang2/MEPRS/computation_time/prscsx/results/")
...
system(paste0("export MKL_NUM_THREADS=1; export NUMEXPR_NUM_THREADS=1; export OMP_NUM_THREADS=1;
              ...
              " --sst_file=",path_to_sum,"EUR.txt,",path_to_sum,"AFR.txt",
              " --n_gwas=100000,15000",
              " --pop=EUR,AFR",
              " --chrom=22",
              " --phi=",phi[k],
...
## match up phenotype
  ...
  SCORE <- fread2(paste0(out.dir,"/two_",eth,"_alltuning.sscore"))
  ...
  fwrite2(best_score_tmp, paste0(out.dir,"/two_",eth,"_best_score.txt"), col.names = T, sep="\t", nThread=1)
...
fwrite2(ensemble_score, paste0(out.dir,"/two_final_score.txt"), col.names = T, sep="\t", nThread=1)

end_time <- Sys.time()

runtime <- as.numeric(end_time-start_time, units="secs")

saveRDS(runtime, file= paste0(out.dir,"/two_runtime.rds"))
