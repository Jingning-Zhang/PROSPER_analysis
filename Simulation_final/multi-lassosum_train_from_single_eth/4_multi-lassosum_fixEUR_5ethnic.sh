...
runr(){
    R CMD BATCH --no-save --no-restore "$1" 4_write_R_multi-lassosum_fixEUR.R 4_write_R_multi-lassosum_fixEUR_setting${setting}_${M}ethnicity_${ethnic1}_rep_${rep}.Rout
}
runr "--args ethnic=${ethnic2} setting='${setting}' rep='${rep}' L='${para}' Lc='${para}' para='${para}'"

cd /dcs04/nilanjan/data/jzhang2/MEPRS/codes/Simulation_final/multi-lassosum_train_from_single_eth/Setting_${setting}/${ethnic1}/4_multi-lassosum_by_chr_${para}_fixEUR
