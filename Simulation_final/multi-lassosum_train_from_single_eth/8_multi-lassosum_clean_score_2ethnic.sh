...
cd /dcs04/nilanjan/data/jzhang2/MEPRS/codes/Pipeline_simulation_multiethnic-lassosum2-parameter/multi-lassosum_train_from_single_eth/Setting_${setting}/${ethnic1}/8_multi-lassosum_clean_score_${para}
...
rm *.po*
rm *.Rout
qsub -l h=${skipnode} -hold_jid six_meprs-${M}ethnicity-${ethnic1}-GA_${setting}_rep_${rep} rep_${rep}.sh
qsub -l h=${skipnode} -hold_jid six_meprs-${M}ethnicity-${ethnic1}-GA_${setting}_rep_${rep}_fixEUR rep_${rep}_fixEUR.sh
