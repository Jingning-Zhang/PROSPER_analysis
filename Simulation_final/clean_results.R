  ...
  for (rho in 1:3){
      for (method in  c(mulmethods,
        ...
        if(mulflag){
          ...
          }else if(method == "multi-lassosum_train_from_single_eth(fixEUR)"){
            tryCatch({
              R2tmp <- tibble()
              for(rep in 1:10){
                tmp <- readRDS(paste0("/dcs04/nilanjan/data/jzhang2/MEPRS/Simulation_final/multi-lassosum_train_from_single_eth/Setting_",setting,"/AFR_AMR_EAS_EUR_SAS/rep_",rep,"/4_multi-lassosum_by_chr_10_fixEUR/Results/test_all_prs/",ethnic,"/rho_",rho,"_size_",size,"_validation_R2.rds"))
                R2tmp <- rbind(R2tmp, tmp)
              }
              R2 <- apply(as.matrix(R2tmp),MARGIN = 2, mean)
            }, error=function(e){})
          }else if(method == "multi-lassosum(fixEUR)3"){
              R2tmp <- tibble()
              for(rep in 1:10){
                tryCatch({
                tmp <- readRDS(paste0("/dcs04/nilanjan/data/jzhang2/MEPRS/Simulation_final/multi-lassosum/Setting_",setting,"/AFR_AMR_EAS_EUR_SAS/rep_",rep,"/4_multi-lassosum_by_chr_3_fixEUR/Results/test_all_prs/",ethnic,"/rho_",rho,"_size_",size,"_validation_R2.rds"))
                R2tmp <- rbind(R2tmp, tmp)
                  }, error=function(e){})
              }
              R2 <- apply(as.matrix(R2tmp),MARGIN = 2, mean)
        ...
        if(!mulflag){
          if(method == "lassosum2"){
            ...
            tryCatch({
              R2tmp <- numeric()
              for(rep in 1:10){
                tmp <- readRDS(paste0("/dcs04/nilanjan/data/jzhang2/MEPRS/Simulation_final/Setting_",setting,"_lassosum2/",ethnic,"/rep_",rep,"/validation_R2_rho_",rho,"_size_4_EUR_best.rds"))
                R2tmp <- c(R2tmp, tmp)
              }
              R2 <- mean(R2tmp)
            }, error=function(e){})
            ...
            tryCatch({
              R2tmp <- tibble()
              for(rep in 1:10){
                tmp <- readRDS(paste0("/dcs04/nilanjan/data/jzhang2/MEPRS/Simulation_final/Setting_",setting,"_lassosum2/",ethnic,"/rep_",rep,"/validation_R2_rho_",rho,"_size_",size,"_weighted_lassosum2.rds"))
                R2tmp <- rbind(R2tmp, tmp)
              }
              R2 <- apply(as.matrix(R2tmp),MARGIN = 2, mean)
            }, error=function(e){})
