...
df_beta <- readRDS(paste0("/dcs04/nilanjan/data/23andme/Analysis/JZ/lassosum2/Results/",targetethnic,"_",trait,"_df_beta.rds"))
...
for (chr in 1:22){
  ...
  load(paste0("/dcs04/nilanjan/data/23andme/Analysis/JZ/multi-lassosum_train_from_single_eth/",ethnic1,"/2_multi-lassosum_by_chr_",para,"/tuning/",trait,"_chr",chr,".RData"))
  delta <- delta_best
  #indx, indx_block, M, snp_list, ethnic,
  load(paste0("/dcs04/nilanjan/data/23andme/Analysis/JZ/multi-lassosum/",ethnic1,"/1_standard_data/",trait,"/chr",chr,".RData"))
  ...
  if(chr==1){
    ...
    bim.ref <- fread2(paste0("/dcs04/nilanjan/data/23andme/Analysis/JZ/ref_geno/",targetethnic,"/",trait,"/ref_chr", chr,".bim"))
...
sumdata <- fread2(paste0("/dcs04/nilanjan/data/23andme/cleaned/",targetethnic,"/sumdat/",trait,"_passQC_noNA_matchinfo_matchMAFnoNA_common_mega+hapmap3_cleaned.txt"))
...
df <- data.frame(SNP = snps, A1= sumdataA1, b_allchr)
fwrite2(df, paste0("/dcs04/nilanjan/data/23andme/Analysis/JZ/multi-lassosum_train_from_single_eth/",ethnic1,"/2_multi-lassosum_by_chr_",para,"/Results/beta_",trait,"_",targetethnic,".txt"), col.names = F, sep="\t", nThread=NCORES)
write_tsv(as.data.frame(param), paste0("/dcs04/nilanjan/data/23andme/Analysis/JZ/multi-lassosum_train_from_single_eth/",ethnic1,"/2_multi-lassosum_by_chr_",para,"/Results/para_",trait,".txt"))

rm(list = c("df","param","sumdata"))

##############################################################
### match to 23andme SNP ID
tmp <- apply(b_allchr, MARGIN=1, function(x){sum(x!=0)}); m <- !(tmp==0)
b_allchr <- b_allchr[m,]
...
df <- data.frame(SNP = id_23andme, b_allchr)
fwrite2(df, paste0("/dcs04/nilanjan/data/23andme/Analysis/JZ/multi-lassosum_train_from_single_eth/prelim_results_table/",M,"ethnicity/",targetethnic,"/",trait,"/prs.file"),
